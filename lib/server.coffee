...
# create http server
http_server = restify.createServer server_options
...
setup_server = (server) ->
	...
	# get providers list
	server.get config.base + '/api/providers', auth.needed, (req, res, next) ->
		db.providers.getList send(res,next)

	# get a provider config
	server.get config.base + '/api/providers/:provider', auth.needed, (req, res, next) ->
		if req.query.extend
	...
	# get a provider config
	server.get config.base + '/api/providers/:provider/logo', auth.needed, ((req, res, next) ->
...
exports.listen = (callback) ->
	...
	plugins.run 'setup', ->
		...
		http_server.listen config.http_port, (err) ->
			return callback err if err
			#exit.push 'Http(s) server', (cb) -> server.close cb
			#/!\ server.close = timeout if at least one connection /!\ wtf?
